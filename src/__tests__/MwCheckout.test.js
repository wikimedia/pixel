...
describe( 'MwCheckout.js', () => {
	...
	describe( 'checkout', () => {
		it( 'calls the maintenance/update.php script at the end', async () => {
			...
			await factory.mwCheckout.checkout( 'master', [ 'Iff231a976c473217b0fa4da1aa9a8d1c2a1a19f2' ] );
		...
		it( 'throws an error when branch is not found', async () => {
			...
			await expect( factory.mwCheckout.checkout( 'branch-that-doesnt-exist', [ 'Iff231a976c473217b0fa4da1aa9a8d1c2a1a19f2' ] ) )
		...
		it( 'throws an error when gerrit returns more than one change', async () => {
			...
			factory.gerritClient.get.mockImplementation( async ( /** @type {string} */ path ) => {
				return path === '/changes/?q=change:Iff231a976c473217b0fa4da1aa9a8d1c2a1a19f2&o=LABELS&o=CURRENT_REVISION&o=CURRENT_COMMIT&o=DOWNLOAD_COMMANDS' ? [ {}, {} ] : gerritResponses( path );
			} );

			await expect( factory.mwCheckout.checkout( 'master', [ 'Iff231a976c473217b0fa4da1aa9a8d1c2a1a19f2' ] ) )
		...
		it( 'throws an error when gerrit returns a change without a revision', async () => {
			...
			factory.gerritClient.get.mockImplementation( async ( /** @type {string} */ path ) => {
				...
				return path === '/changes/?q=change:Iff231a976c473217b0fa4da1aa9a8d1c2a1a19f2&o=LABELS&o=CURRENT_REVISION&o=CURRENT_COMMIT&o=DOWNLOAD_COMMANDS' ?
